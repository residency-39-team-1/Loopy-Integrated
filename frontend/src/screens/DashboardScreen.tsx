  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topBar}>
        <Text style={styles.title}>hello, {user?.displayName?.split(' ')[0] || 'friend'}</Text>
      </View>

      <View style={styles.stats}>
        <Text style={styles.stat}>{activeTasks} active</Text>
        <Text style={styles.stat}>•</Text>
        <Text style={styles.stat}>{tasks.filter(t => t.state === 'Done').length} done</Text>
      </View>

      <View style={styles.grid}>
        <TouchableOpacity style={[styles.tile, styles.primary]} onPress={() => navigation.navigate('TaskList')}>
          <Text style={styles.emoji}>📝</Text>
          <Text style={styles.tileText}>Tasks</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.tile, styles.secondary]} onPress={() => navigation.navigate('Flowboard')}>
          <Text style={styles.emoji}>🎯</Text>
          <Text style={styles.tileText}>Flowboard</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.tile, styles.tertiary]} onPress={() => navigation.navigate('ChaosCatcher')}>
          <Text style={styles.emoji}>🌪️</Text>
          <Text style={styles.tileText}>Chaos</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.tile, styles.quaternary]} onPress={() => navigation.navigate('Progress')}>
          <Text style={styles.emoji}>🏆</Text>
...
const styles = StyleSheet.create({
  ...
  grid: { gap: 12 },
  tile: { height: 100, borderRadius: 16, justifyContent: 'center', alignItems: 'center', elevation: 2 },
